...
- Don't ship a light-mode marketing page. Riffa's native product UI is the
  explicit exception described in “Native application themes” below.
...
## Native application themes

Riffa uses this document's near-black palette as its default **Midnight**
product theme. Unlike the reference marketing page, the native macOS
application also supports System, Light, and Dark appearance preferences.
Theme customization changes semantic tokens only; it must never introduce
gradients, materials, shadows, decorative side colors, or multiple simultaneous
accent colors.

### Theme layers

1. Resolve the requested appearance: follow macOS, Light, or Dark.
2. Select the matching built-in surface and text ladder.
3. Apply one named preset or a validated custom accent.
4. Optionally apply a versioned, partial theme document containing separate
   `light` and `dark` semantic-token overrides.
5. Apply accessibility adjustments such as increased contrast last.

Midnight Dark preserves the canonical values at the top of this document.
Light themes use a warm-neutral canvas around `#f5f6f8`, white lifted panels,
near-black ink, gray hairlines, and the same sparse accent rules. A light theme
is not an inversion filter: every surface, ink, border, focus, and semantic
token must be resolved explicitly.

### Customization constraints

- The active accent is used only for the brand mark, primary action, focus,
  selection, and link emphasis.
- Success, warning, danger, insertion, deletion, and modification semantics are
  not recolored by the accent setting.
- Button foregrounds are selected from the resolved accent luminance so custom
  colors remain readable.
- Presets and swatches always include a visible name and selected indicator;
  color alone never communicates selection.
- Theme documents are size-bounded, schema-versioned, strictly decoded, and
  validated before replacing the active theme. Invalid input keeps the previous
  theme and produces an inline error.
- Theme documents contain paired optional Light/Dark overrides. Missing tokens
  fall back to the chosen built-in palette.
- All AppKit-backed editors receive the same resolved canvas, ink, and selection
  colors as SwiftUI views.
